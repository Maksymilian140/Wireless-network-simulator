...
void ChannelRequestEvent::execute() {
	...
	// if there is then remove client from buffer and plan end of service event for him
		if (is_added) {
			spdlog::info("Time: " + std::to_string(network->clock) + " ##### U" + std::to_string(client->get_group()) + " is added to channel from buffer\n");
			network->buffer_pop();
			float event_t = client->get_service_time() + network->clock;
			Event* service_event;
			service_event = new UserEndOfServiceEvent(event_t, network, client); 
